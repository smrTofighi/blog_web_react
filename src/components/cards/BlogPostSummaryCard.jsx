...
const BlogPostSummaryCard = (
 {
  ...
  return <div className="flex items-start gap-4 bg-white p-3 mb-5 rounded-lg cursor-pointer group" onClick={onClick}>
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="flex items-center gap-1.5 text-xs text-sky-700 font-medium bg-sky-50 px-2.5 py-1 rounded">
          ...
          <div className="flex items-center gap-2.5">
            {tags.map((tag, index)=> (
              <div key={`tag-${index}`} className="text-cyan-700 font-medium bg-cyan-100/50 px-2.5 py-1 rounded">
                {tag}
              </div>
            ))}
          </div>
    ...
    <button className="hidden md:group-hover:flex items-center gap-2 text-xs text-rose-500 font-medium bg-rose-50 px-3 py-1 rounded text-nowrap border border-rose-100 hover:border-rose-300 cursor-pointer" onClick={(e)=>{
      e.stopPropagation()
      onDelete()
    }}>
